  ...
  <div class="app-container">
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            size="small"
            type="text"
            v-if="scope.row.status == 1 && scope.row.createrFlag == 2"
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="checkCourse" v-show="isCheck"
          >确 定</el-button
        >
        <el-button
          type="primary"
...
<script>
import { getSubjectList, getSubjectDetail, changeStatus } from "@/api/manage";
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      ...
      this.progressFlag = [{ isShow: false, loadProgress: 0 }];
    },
    ...
    // 修改按钮
    async hanldeUpdate(row) {
      this.reset();
      this.detailParams.subjectId = row;
      this.title = "修改课程";
      await getSubjectDetail(this.detailParams).then((res) => {
        if (res.code == 200) {
          this.form = res.data;
          if (res.data.category == 1) {
            this.showSignTime = true;
          ...
          if (valid) {
            ...
            if (t3 != "" && t4 != "") {
              if (new Date(t4).getTime() <= new Date(t3).getTime()) {
                this.msgError("课程结束时间选择错误");
                return false;
              }
            }

            if (this.form.id != undefined) {
              this.$store.dispatch("adSaveSubject", this.form).then((res) => {
                this.btnStatus = true;
    ...
    uploadVideoProcess(index, event) {
      ...
      this.progressFlag[index].loadProgress = parseInt(event.percent); // 动态获取文件上传进度
      if (this.progressFlag[index].loadProgress >= 100) {
        this.progressFlag[index].loadProgress = 100;
        setTimeout(() => {
