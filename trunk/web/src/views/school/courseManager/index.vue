      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            ...
            id="8"
            @click="handleUpdate($event, scope.row.id, scope.row.name)"
            v-if="scope.row.status == 7 && scope.row.createrFlag == 1"
            >下架</el-button
          >
          <el-button
            size="small"
...
export default {
  ...
  data() {
    return {
      ...
      progressFlag: [{ isShow: false, loadProgress: 0 }], // 关闭进度条// 动态显示进度条
      //详情参数
      detailParams: {
        subjectId: "",
      },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      schoolServe.getSubjectList(this.queryParams).then((res) => {
        if (res.code == 200) {
