...
public class SysDeptServiceImpl implements ISysDeptService {
    ...
    public List<SysDept> selectSchoolList() {
        List<SysDept> list = redisCache.getCacheObject(Constants.HIGHSCHOOLS_KEY);
        if (CollectionUtils.isNotEmpty(list)) {
            return list;
        } else {
            list = deptMapper.selectSchoolList();
            if (CollectionUtils.isNotEmpty(list)) {
                redisCache.setCacheObject(Constants.HIGHSCHOOLS_KEY, list, Constants.BUSSINESS_DATA_CACHE, TimeUnit.MINUTES);
            }
        }
        return list;
    }
