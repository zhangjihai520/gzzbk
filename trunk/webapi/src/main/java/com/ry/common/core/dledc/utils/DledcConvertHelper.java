...
import com.ry.common.core.dledc.response.ClassInfo;
import com.ry.common.enums.UserRoleEnum;
...
public class DledcConvertHelper {
    ...
    public static int ConvertUserType(String userType) {
        ...
        switch (userType) {
            case "teacher":
                result = (int) UserType.Teacher.getId();
                break;
            case "student":
                result = (int) UserType.Student.getId();
                break;
        }
        return result;
    }
    ...
    public static String ConvertUserRoles(List<String> sourseUserRoles) {
        if (CollectionUtils.isEmpty(sourseUserRoles) || sourseUserRoles.size() == 0) {
            return StringUtils.EMPTY;
        }
